...
const TotalMoney = ({ transactions, selectedMonth }) => {
    ...
    useEffect(() => {
      const newMonthTotals = transactions.reduce((acc, transaction) => {
  ...
  return (
    <TotalContainer>
      ...
      <TotalText>Total Expense: <span style={{ color: '#FF6928' }}>${monthTotals.expense}</span></TotalText>
      <TotalText>Total Income: <span style={{ color: '#28a745' }}>${monthTotals.income}</span></TotalText>
    </TotalContainer>
  );
};
